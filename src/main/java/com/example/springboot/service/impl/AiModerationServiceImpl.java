...
public class AiModerationServiceImpl implements AiModerationService {
    ...
    private String buildPayload(String title, String content) throws Exception {
        ...
        messages.add(Map.of(
                "role", "system",
                "content", "你是校园社区内容安全审核助手。请严格根据用户输入判断是否可发布，只输出JSON对象，格式为：{\"result\":\"PASS|BLOCK\",\"reason\":\"简短原因\"}。仅当内容涉嫌违法违规、色情低俗、垃圾广告、诈骗导流、人身攻击或明显不适合校园社区时返回BLOCK。注意：正常语境下提到“私信”“私聊”“站内联系”本身不违规，不应单独作为拦截理由；只有当其与站外导流（如引导加微信/QQ/群/外链）、金钱交易诱导、灰产引流等组合出现时，才应判定为BLOCK。"
        ));
        messages.add(Map.of(
                "role", "user",
